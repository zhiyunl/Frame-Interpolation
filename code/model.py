...
class KernelEstimation(torch.nn.Module):
    def __init__(self, kernel_size):
        ...
        def Base(input_channel, output_channel):
        ...
        self.moduleConv1 = Base(6, 32)
        self.modulePool1 = torch.nn.AvgPool2d(kernel_size=2, stride=2)

        self.moduleConv2 = Base(32, 64)
        self.modulePool2 = torch.nn.AvgPool2d(kernel_size=2, stride=2)

        self.moduleConv3 = Base(64, 128)
        self.modulePool3 = torch.nn.AvgPool2d(kernel_size=2, stride=2)

        self.moduleConv4 = Base(128, 256)
        self.modulePool4 = torch.nn.AvgPool2d(kernel_size=2, stride=2)

        self.moduleConv5 = Base(256, 512)
        self.modulePool5 = torch.nn.AvgPool2d(kernel_size=2, stride=2)

        self.moduleDeconv5 = Base(512, 512)
        self.moduleUpsample5 = Upsample(512)

        self.moduleDeconv4 = Base(512, 256)
        self.moduleUpsample4 = Upsample(256)

        self.moduleDeconv3 = Base(256, 128)
        self.moduleUpsample3 = Upsample(128)

        self.moduleDeconv2 = Base(128, 64)
        self.moduleUpsample2 = Upsample(64)

        self.moduleVertical1 = SeparableNet(self.kernel_size)
        self.moduleVertical2 = SeparableNet(self.kernel_size)
        self.moduleHorizontal1 = SeparableNet(self.kernel_size)
        self.moduleHorizontal2 = SeparableNet(self.kernel_size)
...
class AdaptiveConvNet(torch.nn.Module):
    def __init__(self, kernel_size, lr=0.001):
    ...
    def forward(self, frame0, frame2):
        ...
        models = adaptiveConv.FunctionSepconv().apply
        tensorDot1 = models(self.modulePad(frame0), Vertical1, Horizontal1)
        tensorDot2 = models(self.modulePad(frame2), Vertical2, Horizontal2)
    ...
    def test_model(self, frame0, frame2, frame1):
        output = self.forward(frame0, frame2)
        loss = self.criterion(output, frame1)
        return loss,output
